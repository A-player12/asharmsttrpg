...
- **Tactic upgrades**: `frontline`, `assault`, and `defender` tier-2 data
  is in (`TACTIC_UPGRADES` in `gamedata.js`). `firesupport`'s upgrade data
  got cut off mid-paste (no skills, incomplete bonusStats) and `ambush`/
  `coverfire`/`artillery` have no upgrade data at all — the sheet shows a
  warning instead of crashing if you flag one of those as unlocked.
